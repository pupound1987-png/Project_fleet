      ...
      <SidebarInset className="bg-background">
        <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between gap-2 border-b bg-white/80 backdrop-blur-md px-4 sm:px-6">
          <div className="flex items-center gap-2">
            <SidebarTrigger />
            <h2 className="text-lg font-semibold text-blue-900">Fleet Calendar | ปฏิทินการใช้รถ</h2>
          </div>
          <div className="flex items-center gap-2">
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-accent/30">
                      <th className="p-4 text-left font-bold text-blue-900 border-b min-w-[180px]">Vehicle | ยานพาหนะ</th>
                      {days.map(day => (
                    ...
                    {vehicles?.map(vehicle => (
                      <tr key={vehicle.id} className="hover:bg-blue-50/30 transition-colors">
                        <td className="p-4 border-b">
                          <div className="font-semibold text-blue-900">{vehicle.vehicleName}</div>
                          <div className="text-xs text-muted-foreground font-mono">{vehicle.licensePlate}</div>
                        </td>
                        {days.map(day => {
                              ...
                              <div className="space-y-1">
                                {dayBookings.map(b => (
                                  <div key={b.id} className="p-1.5 bg-primary/20 rounded border border-primary/30 text-[10px] leading-tight">
                                    <div className="font-bold text-blue-900 truncate">{b.employeeName}</div>
                                    <div className="truncate text-muted-foreground">{b.destination}</div>
                                    <div className="mt-1 text-[9px] font-mono">
                                      {b.startDateTime && b.endDateTime ? `${format(new Date(b.startDateTime), 'HH:mm')} - ${format(new Date(b.endDateTime), 'HH:mm')}` : ''}
                                    </div>
                                  </div>
                                ))}
